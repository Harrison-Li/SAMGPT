...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        num = int(config.scaffold_maxlen)   #int(config.lstm_layers)    #  int(config.scaffold) 
        # num = 1
        max_seq_len = config.block_size + num + len(config.cond_props)
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.lstm:
            self.lstm = nn.LSTM(input_size = config.n_embed, hidden_size = config.n_embed, num_layers = config.lstm_layers, dropout = 0.3, bidirectional = False)
