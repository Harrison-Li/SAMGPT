...
if __name__ == '__main__':
    ...
    parser.add_argument('--run_name', type=str, help="name for wandb run", required=False, default="scaf_tl")
    ...
    parser.add_argument('--block_size', type=int, default=100,help="block size", required=False)
    ...
    parser.add_argument('--output_ckpt', type=str, help="path to save the fine-tuned model", required=False, default='tl_scaf_guacamol2_2.pt')

    args = parser.parse_args()

    set_seed(42)
    wandb.init(project="tl_moses_scaf_100_fe", name=args.run_name)

    data = pd.read_csv(f'datasets/{args.data_name}.csv')
    # Clean the missing values by removing the rows.
    data = data.dropna(axis=0).reset_index(drop=True)
    data.columns = data.columns.str.lower()
    # Delete the data which length is larger than the constrain.
    pattern =  "(\[[^\]]+]|<|Br?|Cl?|N|O|S|P|F|I|b|c|n|o|s|p|\(|\)|\.|=|#|-|\+|\\\\|\/|:|~|@|\?|>|\*|\$|\%[0-9]{2}|[0-9])"
    regex = re.compile(pattern)
    lens=[len(regex.findall(i.strip())) for i in data['smiles'] if isinstance(i, str)]
    data['smiles_len']=lens
    data=data[data['smiles_len']<=100].reset_index(drop=True)
    ...
    pretrained_state_dict = torch.load(args.pretrained_model_weight)

    # Get new model's state_dict
    model_state_dict = model.state_dict()

    # Filter out unnecessary keys and copy the matching weights
    ...
    #for param in model.blocks[0:].parameters():  # Example: Unfreezing the ahead 3 blocks
        #param.requires_grad = False
    
    # Optionally freeze pre-trained layers
    for name, param in model.named_parameters():
        if 'blocks.' in name and int(name.split('.')[1]) < 8:
            param.requires_grad = False
